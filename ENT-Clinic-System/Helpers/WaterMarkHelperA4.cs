...
{
    internal class WaterMarkHelperA4
    {
        /// <summary>
        /// Prints the clinic header with title, subtitle, and columned information for A4 page.
        ...
        public static int PrintHeader(Graphics g, int leftMargin, int startY, int pageWidth)
        {
            ...
            using (Font subtitleFont = new Font("Arial", 10, FontStyle.Regular))
            using (Font columnTitleFont = new Font("Arial", 10, FontStyle.Bold))
            using (Font columnFont = new Font("Arial", 10, FontStyle.Regular))
            {
        ...
        /// <summary>
        /// Prints the report footer and returns the new Y position for A4 page.
        /// </summary>
        public static int PrintFooter(Graphics g, int leftMargin, int startY, int pageWidth)
        {
            ...
            using (Font numberFont = new Font("Arial", 10, FontStyle.Bold))
            using (Pen linePen = new Pen(Color.Black, 1))
            {
                int colX = pageWidth - 50;

                g.DrawString(clinicName, nameFont, Brushes.Black, colX - 200, y);
                y += 24;

                // License number
                g.DrawString("Lic. No.", labelFont, Brushes.Black, colX - 200, y);
        ...
        private static void DrawWrappedText(Graphics g, string text, Font font, int x, int y, int maxWidth)
        {
            ...
            RectangleF rect = new RectangleF(x, y, maxWidth, 2000); // Increased height for A4
